...
def verify(sol: object) -> None:
    ...
    eps_Li_x = np.hstack(
        [
            an.eps_el * Li_el_an * (an.xp - an.xm),
    ...
    for i, it in enumerate(t_inds):
        ax.plot(
            x * 1e6, postvars["sum_ip"][it, :] + exp["i_ext"], color=cmap(i)
        )
    ...
    ax.vlines(
        an.xp[-1] * 1e6, 0.95 * ymin, 1.05 * ymax, "k", linestyles="dashed"
    )
    ax.vlines(
        sep.xp[-1] * 1e6, 0.95 * ymin, 1.05 * ymax, "k", linestyles="dashed"
    )
