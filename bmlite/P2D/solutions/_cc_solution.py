...
class CCSolution(BaseSolution):
    ...
    def verify(self, plotflag: bool = False, rtol: float = 5e-3,
        ...
        from ..postutils import _liquid_phase_Li, _solid_phase_Li, current
        ...
        checks.append(np.allclose(i_ext, -sum_ip, rtol=rtol, atol=atol))
        ...
        if plotflag:
            ...
            ax[0, 1].plot(self.t, i_ext + i_an, '-C3')
            ax[0, 2].plot(self.t, i_ext - i_ca, '-C2')
            ...
            ax[1, 0].plot(self.t, i_ext + self.postvars['sum_ip'])
