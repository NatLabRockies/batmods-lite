...
class Gen2Electrolyte(object):
    ...
    def get_D(self, C_Li: float | _ndarray, T: float) -> float | _ndarray:
        ...
        D = 0.0001 * 10 ** (
            ...
            + (-0.810872100 + 475.291 / (T - (-24.83763 + 64.07366 * C_Li)))
            * C_Li
            + (-0.005192312 - 33.43827 / (T - (-24.83763 + 64.07366 * C_Li)))
            * C_Li**2
        )
    ...
    def get_t0(self, C_Li: float | _ndarray, T: float) -> float | _ndarray:
        ...
        t0 = (
            (-0.0000002876102 * T**2 + 0.0002077407 * T - 0.03881203)
            * C_Li**2
    ...
    def get_kappa(self, C_Li: float | _ndarray, T: float) -> float | _ndarray:
        ...
        A = np.array(
            [
                [0.0, 0.0, 1.909446e-4, -8.038545e-2, 9.003410e0],
                [
                    -2.8875870e-8,
                    3.483638e-5,
                    -1.583677e-2,
    ...
    def get_gamma(self, C_Li: float | _ndarray, T: float) -> float | _ndarray:
        ...
        gamma = (
            0.540 * np.exp(329 / T) * C_Li**2
            - 0.00225 * np.exp(1360 / T) * C_Li
            + 0.341 * np.exp(261 / T)
        )
