...
def test_post_verify(CCsim):
    ...
    eps_Li_x = np.hstack(
        [
            Li_el_an * an.eps_el * (an.xp - an.xm),
    ...
    assert np.allclose(CCsim.post_vars["i_el_x"][:, 0], 0.0)
    assert np.allclose(CCsim.post_vars["i_el_x"][:, -1], 0.0)
